...
int main(int argc, char **argv)
{
	...
	while(SDL_PollEvent(&event) >= 0) {
		...
		if(event.type == SDL_KEYDOWN) {
			if(event.key.keysym.sym==SDLK_LEFT) cur_pal = (cur_pal - 1) % 5;
			if(event.key.keysym.sym==SDLK_RIGHT) cur_pal = (cur_pal + 1) % 5;
		}

		pallet_blit_SDL(srfpal, blit_src, im_w, im_h, pals[cur_pal]);
